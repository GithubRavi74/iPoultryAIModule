...
if submitted:
    try:
        ...
        # -------------------------
        # Post-process mortality & FCR to be realistic
        # -------------------------
        # Convert raw lists to numpy arrays for easier ops
        weight_arr = np.array(weight_preds, dtype=float)
        mort_raw = np.array(mort_preds, dtype=float)   # raw model outputs (may be floats)
        fcr_raw = np.array(fcr_preds, dtype=float)     # raw model outputs

        n_days = len(weight_arr)

        # 1) Make mortality integers, non-negative
        mort_daily = np.maximum(np.round(mort_raw), 0).astype(int)

        # 2) Simulate remaining birds so daily mortality cannot exceed remaining flock
        birds_remaining = int(birds_alive)
        mort_final = []
        for i in range(n_days):
            daily = int(mort_daily[i])
            # Cap daily deaths to remaining birds
            daily = min(daily, birds_remaining)
            mort_final.append(daily)
            birds_remaining -= daily
            if birds_remaining < 0:
                birds_remaining = 0

        # 3) Compute FCR per day from feed and weight gain (preferred)
        # Assumptions:
        #  - feed_today is total feed for the flock (kg) for the baseline day you provided.
        #  - We assume roughly same feed per bird unless you have daily feed schedule.
        #  - We'll compute per-bird feed using feed_today / birds_alive and keep constant for demo.
        feed_per_bird = (feed_today / birds_alive) if birds_alive > 0 else 0.0

        # Create list to hold final FCR values
        fcr_final = []
        prev_weight = float(sample_weight_today)

        # realistic min/max bounds
        FCR_MIN, FCR_MAX = 1.2, 2.5
        MIN_WEIGHT_GAIN = 0.005  # 5 grams min sensible daily gain fallback

        for i in range(n_days):
            pred_w = float(weight_arr[i])
            # weight gain per bird from previous to this day
            gain = pred_w - prev_weight

            # if gain is tiny or negative, fallback to small positive gain to avoid division by zero
            if gain < MIN_WEIGHT_GAIN:
                gain_use = MIN_WEIGHT_GAIN
            else:
                gain_use = gain

            # per-day feed per bird (kept same here; you can change to dynamic if you have per-day feed)
            feed_pb = feed_per_bird

            # compute FCR = feed per bird for the day / weight gain per bird that day
            computed_fcr = feed_pb / gain_use if gain_use > 0 else FCR_MAX

            # If computed_fcr is outside realistic bounds, clamp it
            computed_fcr = max(FCR_MIN, min(computed_fcr, FCR_MAX))

            # Use model FCR only if it is within bounds and looks more sensible (optional)
            model_fcr = float(fcr_raw[i]) if i < len(fcr_raw) else None
            if model_fcr and (FCR_MIN <= model_fcr <= FCR_MAX):
                # Blend model FCR and computed one to preserve some model signal
                final_fcr = 0.4 * model_fcr + 0.6 * computed_fcr
            else:
                final_fcr = computed_fcr

            fcr_final.append(round(final_fcr, 3))
            prev_weight = pred_w

        # 4) Replace lists used for dataframe
        mort_preds = mort_final
        fcr_preds = fcr_final

        # Optional: sanity-check totals
        total_predicted_deaths = sum(mort_preds)
        if total_predicted_deaths > birds_alive:
            # scale down proportionally (shouldn't normally happen due to earlier capping)
            scale = birds_alive / total_predicted_deaths
            mort_preds = [int(np.floor(m * scale)) for m in mort_preds]
        ...
        df_forecast = pd.DataFrame({
            ...
            "Predicted Mortality (birds)": np.array(mort_preds).astype(int),
            "Predicted FCR": np.array(fcr_preds)
        })
        ...
        st.markdown(
        f"""
        <h2 style="color:#27AE60;">Day 35 Prediction- &nbsp;Weight:{w35:.2f} kg</h2>
        """,
        unsafe_allow_html=True
       )
